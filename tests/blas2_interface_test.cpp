...
/* void _gemv0(Executor<ExecutorType> ex, std::string _Trans, size_t _M, size_t
 * _N, */
...
/*   auto my_mA = */
/*       matrix_view<T, ContainerT>(_mA, _M, _N, accessOpr, _lda,
 * _mA.getDisp()); */
 ...
 */
/*       auto my_rs = vector_view<T, ContainerT>(my_vy, i + my_vy.getDisp(), 1,
 * 1); */
...
void _ger0(Device &dev, size_t _M, size_t _N, T _alpha,
  ...
  if (my_mA.getAccess()) {
    ...
    for (size_t i = 0; i < M; i++) {
      auto my_row = vector_view<T, ContainerT>(my_mA.getData(), disp, 1, N);
      auto scalExpr2 =
          make_expr<ScalarExpr, prdOp2_struct>(_alpha * my_vx.eval(i), my_vy);
      auto addExpr2 = make_expr<BinaryExpr, addOp2_struct>(my_row, scalExpr2);
      auto assignExpr2 = make_assign(my_row, addExpr2);
      blas::execute(assignExpr2);
      disp += _lda;
    }
  } else {
    auto disp = my_mA.getDisp();
    for (size_t j = 0; j < N; j++) {
      auto my_col = vector_view<T, ContainerT>(my_mA.getData(), disp, 1, M);
      auto scalExpr2 =
          make_expr<ScalarExpr, prdOp2_struct>(_alpha * my_vy.eval(j), my_vx);
      auto addExpr2 = make_expr<BinaryExpr, addOp2_struct>(my_col, scalExpr2);
      auto assignExpr2 =
          AssignExpr<decltype(my_col), decltype(addExpr2)>(my_col, addExpr2);
      blas::execute(assignExpr2);
      disp += _lda;
    }
  }
...
size_t TestingBLAS2(bool accessDev, size_t dim, size_t divSz, size_t shftR,
...

  {
    ...
    _gemv(dev, "No", dimR - shftR, dimC - shftC, 2.0, bmM0(shftR, shftC), dimL,
          bvX0, 1, 1.5, bvY0, 1);
    _gemv(dev, "Tr", dimC - shftC, dimR - shftR, 2.5, bmM0(shftR, shftC), dimL,
          bvY0, 1, 0.5, bvX0, 1);
