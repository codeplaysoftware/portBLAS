...
typename std::enable_if<Gemm::version == 2>::type test(
    int r, int m, int n, int k, T alpha, const Container &dataA, int lda,
            ...
            << " ===" << std::endl;
  {
    ...
    run_test(r, 2.0 * m * n * k, [&] {
      q.submit([&](handler &cgh) {
        ...
        cgh.parallel_for<Test<Gemm>>(
            Gemm::get_nd_range(m, n), [=](nd_item<1> id) {
...
template <typename Gemm, typename T, typename Container>
typename std::enable_if<Gemm::version == 19>::type test(
    int r, int m, int n, int k, T alpha, const Container &dataA, int lda,
    const Container &dataB, int ldb, T beta, Container dataC, int ldc,
    const Container &refC, cl::sycl::queue q) {
  using etype = typename Gemm::value_type;
            ...
            << " ===" << std::endl;
  {
    ...
    run_test(r, 2.0 * m * n * k, [&] {
      q.submit([&](handler &cgh) {
        ...
        cgh.parallel_for<Test<Gemm>>(
            Gemm::get_nd_range(m, n), [=](nd_item<1> id) {
              Gemm::run(id, id.get_group(0), id.get_local(0), m, n, k,
                        etype(alpha), accA.get_pointer(), lda,
                        accB.get_pointer(), ldb, etype(beta),
...
void test_syclblas(int r, char transA, char transB, int m, int n, int k,
                   ...
                   int ldc, const Container &refC, cl::sycl::queue q) {
  ...
  Executor<SYCL> ex(q);
  {
    ...
    buffer<etype, 1> buffC(dataC.data(), range<1>(dataC.size()));
    run_test(r, 2.0 * m * n * k, [&] {
      _gemm(ex, transA, transB, m, n, k, alpha, buffA, lda, buffB, ldb, beta,
            buffC, ldc);
      q.wait();
    });
  }
...
#define ARG m, n, k, E(1), dataA, lda, dataB, ldb, E(1), origC, ldc, refC, q
...
#define TARG(_tir, _tic, _twr, _twc, _ttr, _ttc)                             \
  GemmFactory<db, ba, bb, cls, Tile<_tir, _tic, _twr, _twc, _ttr, _ttc>, ta, \
              tb, E>
