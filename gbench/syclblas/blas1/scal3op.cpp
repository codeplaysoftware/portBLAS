...
void BM_Scal3op(benchmark::State& state) {
  ...
  std::vector<ScalarT> v3 = benchmark::utils::random_data<ScalarT>(size);
  ...
  auto inz = blas::helper::make_sycl_iterator_buffer<ScalarT>(v3, size);
  ...
  for (auto _ : state) {
    ...
    cl::sycl::event event2 = _scal(ex, size, alpha, inz, 1);
