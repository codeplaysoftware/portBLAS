...
void BM_Scal3op(benchmark::State& state) {
  ...
  MemBuffer<scalar_t> buf2(ex, v2.data(), size);
  MemBuffer<scalar_t> buf3(ex, v3.data(), size);
...
BENCHMARK_TEMPLATE(BM_Scal3op, float)->RangeMultiplier(2)->Range(2 << 5, 2 << 18);
#ifdef DOUBLE_SUPPORT
BENCHMARK_TEMPLATE(BM_Scal3op, double)->RangeMultiplier(2)->Range(2 << 5, 2 << 18);
#endif
