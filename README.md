...
| `_rotg`   | `sb_handle`, `a`, `b`, `c`, `s`                        | Given the Cartesian coordinates (`a`, `b`) of a point, return the parameters `c`, `s`, `r`, and `z` associated with the Givens rotation. Note: this uses netlib blas as a reference. OpenBLAS can give different output for some edge cases and fail in some tests. |
| `_rotm`   | `sb_handle`, `N`, `vx`, `incx`, `vy`, `incy`, `param`  | Applies a modified Givens rotation to `x` and `y`.                                                                                                                           |
| `_rotmg`  | `sb_handle`, `d1`, `d2`, `x1`, `y1` `param`            | Given the Cartesian coordinates (`x1`, `y1`) of a point, return the components of a modified Givens transformation matrix that zeros the y-component of the resulting point. Note: this uses netlib blas as a reference. OpenBLAS can give different output for some edge cases and fail in some tests. |
...
| `_iamax`  | `sb_handle`, `N`, `vx`, `incx` [, `rs`]                | Index of the first occurrence of the maximum element in `x`; written to `rs` if passed, else returned.                                                              |
| `_iamin`  | `sb_handle`, `N`, `vx`, `incx` [, `rs`]                | Index of the first occurrence of the minimum element in `x`; written to `rs` if passed, else returned.                                                            |
