...
Executor<PolicyHandler<codeplay_policy>>::execute(
  ...
  const index_t rows = gemm_wrapper.m, cols = gemm_wrapper.n;

  /* First step: partial gemm */

  /* Create the cube that is generated by the partial gemm and turned into a
   * matrix by the reduction operation */
  auto cube_buffer = make_sycl_iterator_buffer<element_t>(rows * cols * depth);
  auto cube =
      make_matrix_view<col_major>(*this, temp_buffer, rows * cols, depth, rows);
