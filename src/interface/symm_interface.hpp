...
typename sb_handle_t::event_t _symm(sb_handle_t& sb_handle, char _side,
  ...
  if (_side == SIDE_LEFT) { // C <- alpha * A * B + beta * C
    ...
    return _gemm_backend<true, false>(sb_handle, trans_symm, TRANS_NO,
                       _M, _N, _M, _alpha, a_, _lda, index_t(0),
                       b_, _ldb, index_t(0), _beta, _C, _ldc, index_t(0),
    ...
    return _gemm_backend<false, true>(sb_handle, TRANS_NO, trans_symm,
                       _M, _N, _N, _alpha, b_, _ldb, index_t(0),
                       a_, _lda, index_t(0), _beta, _C, _ldc, index_t(0),
                       index_t(1), gemm_batch_type_t::strided);
  } else {
