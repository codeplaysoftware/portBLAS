...
typename Executor::Policy::event_type _gemv_legacy_impl(
  ...
  typename Executor::Policy::event_type ret;
  ...
  auto valT1 = blas::make_sycl_iterator_buffer<T>(M * scratchSize);
...
template <typename Executor,       // The type of the executor (e.g. sycl, etc)
          ...
          >
typename Executor::Policy::event_type _gemv_impl(
...
typename Executor::Policy::event_type _trmv_impl(
  ...
  auto valT1 = blas::make_sycl_iterator_buffer<T>(N * scratchSize);
...
template <typename Executor, typename IndexType, typename T,
          typename ContainerT0, typename ContainerT1, typename IncrementType,
          typename ContainerT2>
typename Executor::Policy::event_type _symv_impl(
  ...
  auto valTR = blas::make_sycl_iterator_buffer<T>(N * scratchSize_R);
  auto matR = make_matrix_view(ex, valTR, N, scratchSize_R, scratchSize_R,
                               Access::ColMajor());

  const IndexType scratchSize_C = nWGPerCol_C;

  auto valTC = blas::make_sycl_iterator_buffer<T>(N * scratchSize_C);
...
template <typename Executor, typename IndexType, typename T,
          ...
          typename ContainerT2>
typename Executor::Policy::event_type _ger_impl(
...
template <typename Executor, typename IndexType, typename T,
          typename ContainerT0, typename IncrementType, typename ContainerT1>
typename Executor::Policy::event_type _syr_impl(
...
template <typename Executor, typename IndexType, typename T,
          typename ContainerT0, typename IncrementType, typename ContainerT1,
          typename ContainerT2>
typename Executor::Policy::event_type _syr2_impl(
    Executor& ex, char _Uplo, IndexType _N, T _alpha, ContainerT0 _vx,
    IncrementType _incx, ContainerT1 _vy, IncrementType _incy, ContainerT2 _mA,
    IndexType _lda, IndexType _localSize = 0, IndexType _scratchPadSize = 0,
    IndexType _nRowsWG = 0, IndexType _nColsWG = 0) {
