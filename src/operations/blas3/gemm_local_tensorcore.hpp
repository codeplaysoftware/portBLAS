...
class Gemm<input_t, output_t, DoubleBuffer, NbcA, NbcB, ClSize, TileType,
 ...
 public:
  ...
  static constexpr index_t item_rows = tile_type::item_rows;//2
  static constexpr index_t item_cols = tile_type::item_cols;//4
  static constexpr index_t wg_rows = tile_type::wg_rows;//1
  static constexpr index_t wg_cols = tile_type::wg_cols;//1
  static constexpr index_t sg_rows = tile_type::sg_rows;//16
  static constexpr index_t sg_cols = tile_type::sg_cols;//16
  static constexpr index_t tl_rows = tile_type::tl_rows;//1
  static constexpr index_t tl_cols = tile_type::tl_cols;//1
  static constexpr index_t tile_size = tl_rows * tl_cols;//1
  static constexpr index_t sg_size = 32;

  static constexpr bool double_buffer = DoubleBuffer;//1
  static constexpr bool nbc_a = NbcA;//0
  static constexpr bool nbc_b = NbcB;//1
  static constexpr bool trans_a = TransA;//0
  static constexpr bool trans_b = TransB;//0
  ...
  static constexpr index_t big_tile_cols = tl_cols * block_cols;//16
   ...
   *
   */
  SYCL_BLAS_INLINE index_t get_sg_x_cluster() const noexcept {
    return ((a_.get_size_row() - 1) / sg_rows + 1);
  }

  SYCL_BLAS_INLINE index_t get_sg_y_cluster() const noexcept {
    return ((b_.get_size_col() - 1) / sg_cols + 1);
  }
  /*!
   *@brief get_batch_wg_cluster. This function is used to extend the number
   ...
   *
   */
  SYCL_BLAS_INLINE size_t
  get_batch_wg_cluster(index_t compute_units) const noexcept {
    return static_cast<size_t>((4 * compute_units - 1) / batch_size_ + 1);
  }
   ...
   */
  SYCL_BLAS_INLINE cl::sycl::nd_range<3> get_nd_range(
      index_t compute_units) const noexcept {
    size_t batch_groups = static_cast<size_t>(batch_size_ * get_batch_wg_cluster(compute_units));
...
#endif
    return cl::sycl::nd_range<3>{{batch_groups, x_groups * x_local, y_groups * y_local},
                                 {1, x_local, y_local},
                                 {}};
  }
  ...
  SYCL_BLAS_INLINE void eval(local_memory_t scratch_acc,
                             const cl::sycl::nd_item<3> &id) noexcept {
    // cl::sycl::ext::oneapi::experimental::printf("%lu | %lu | %d | %d\n", id.get_group_range(0), id.get_group_range(1), get_sg_x_cluster(), get_sg_y_cluster());
    // cl::sycl::ext::oneapi::experimental::printf("%d | %d | %d | %d\n", a_.get_size_row(), sg_rows, b_.get_size_col(), sg_cols);
    ...
    const index_t wg_batch_id = static_cast<index_t>(id.get_group(0));
    // This will disable all workgroups that dont have any batch to work on
    if (wg_batch_id >= batch_size_) {
      return;
    }
    const index_t batch_stride = id.get_group_range(0) / batch_size_;

    auto scratch = scratch_acc.localAcc.get_pointer();

    // The number of work-group required to executed each batch efficiently
    const index_t wg_id_x = id.get_group(1) % x_groups;
    const index_t wg_id_y = id.get_group(2) % y_groups;
    ...
    ptr_B += (trans_b ? (item_id / block_cols) * ldb +
                      ...
                      : item_id % cl_elems +
                            (wg_col + item_id / cl_elems) * ldb);

    n = n - wg_col -
        (trans_b ? item_id % block_cols : item_id / cl_elems);
    ptr_A += (trans_a ? (wg_row + item_id / cl_elems) * lda +
                            (item_id % cl_elems)
                      : (wg_row + item_id % block_rows) +
                            (item_id / block_rows) * lda);

    m = m - wg_row -
        (trans_a ? item_id / cl_elems : item_id % block_rows);

    const index_t s1_offset =
        (trans_b ? item_id / block_cols + (item_id % block_cols) * ldsb
                 : item_id % cl_elems + (item_id / cl_elems) * ldsb);
    const index_t s2_offset = (sg_id / wg_rows) * sg_cols * ldsb;
    ...
    const index_t s4_offset = ofs + (sg_id % wg_rows) * sg_rows;

    if constexpr (std::is_same<typename tile_type::jmInpType, cl::sycl::ext::oneapi::experimental::matrix::precision::tf32>::value) {
      ...
      using address_t = cl::sycl::access::address_space;
      auto input_scratch = *reinterpret_cast<cl::sycl::multi_ptr<typename tile_type::jmInpType, address_t::local_space>*>(&scratch);
  ...
  SYCL_BLAS_INLINE bool valid_thread(const cl::sycl::nd_item<3> &ndItem) const {
    return true;
  }
 ...
 private:
  ...
  SYCL_BLAS_INLINE void compute_panel_gemm(
      const cl::sycl::nd_item<3> &id, const index_t &item_id, const index_t &m,
      ...
      const index_t &ldc, OutputScratchPointerType s0, InputScratchPointerType s1, 
      InputScratchPointerType s2, InputScratchPointerType s3, InputScratchPointerType s4, 
    ...
    using CType = joint_matrix<typename tile_type::jmOutType, matrix_use::accumulator, sg_rows, sg_cols, matrix_layout::col_major>;
    do {
      auto A = orig_A;
      auto B = orig_B;
      auto C = orig_C;
  ...
  /*!
   * @brief Store the computed gemm result to the C matrix after performing input
   * and output scaling. Output scaling only takes place if beta is non-zero.
   *
   * @tparam check_m_limit  iff true, check if row indexes of C are
   *                        out-of-bound
   * @tparam check_n_limit  iff true, check if no indexes of C are
  ...
  SYCL_BLAS_INLINE void store_output_block(cl::sycl::nd_item<3> id, index_t mc, index_t nc,
                                           OutputPointerType C, ScratchPointerType scratch,
                                           index_t ldc, CType& reg_res,
                                           const bool out_of_range) noexcept {
    if (out_of_range) {
      return;
    }
    using namespace cl::sycl::ext::oneapi::experimental::matrix;
    using Cfloat_Type = joint_matrix<element_t, matrix_use::accumulator, sg_rows, sg_cols, matrix_layout::col_major>;
    ...
    const index_t sg_mc = mc - (sg_id % wg_rows) * sg_rows;
    const index_t sg_nc = nc - (sg_id / wg_rows) * sg_cols;
    const bool jm_store_feasible = (mc < block_rows || nc < block_cols) ?
                                    (sg_mc >= sg_rows && sg_nc >= sg_cols) ? 
                                    true : false :
                                    true;

    if (jm_store_feasible) {
      // cl::sycl::ext::oneapi::experimental::printf("jm_store_feasible for sg_id:%d | item_id:%d | mc:%d | nc:%d\n", sg_id, sg_item_id, sg_mc, sg_nc);

      const index_t loop_limit = (sg_rows * sg_cols) / sg_size;

      if constexpr (is_beta_zero) {

        for (index_t i = 0; i < loop_limit; i++) {
          element_t data_left = static_cast<element_t>(reg_res.get_wi_data()[i]);
          float_out.get_wi_data()[i] = alpha_ * data_left;
        }

        joint_matrix_store(sg, float_out, C, ldc);
      } else {

        joint_matrix_load(sg, float_out, C, ldc);

        for (index_t i = 0; i < loop_limit; i++) {
          element_t data_left = static_cast<element_t>(reg_res.get_wi_data()[i]);
          element_t data_right = float_out.get_wi_data()[i];
          float_out.get_wi_data()[i] = beta_ * data_right + alpha_ * data_left;
        }
    ...
    scratch += sg_id * sg_rows;

    const index_t sg_store_ld = sg_range * sg_rows;

    for (index_t i = 0; i < (sg_rows * sg_cols) / sg_size; i++) {
      float_out.get_wi_data()[i] = static_cast<element_t>(reg_res.get_wi_data()[i]);
    }

    joint_matrix_store(sg, float_out, scratch, sg_store_ld);

    id.barrier(cl::sycl::access::fence_space::local_space);

    const index_t loop_limit = sg_nc >= sg_cols ? sg_cols : sg_nc;

    C += sg_item_id;
    scratch += sg_item_id;

    if (sg_mc < sg_rows && sg_nc < sg_cols) {
      if (sg_item_id < sg_mc) {
        // cl::sycl::ext::oneapi::experimental::printf("sg_id:%d | item_id:%d | mc:%d | nc:%d\n", sg_id, sg_item_id, sg_mc, sg_nc);

        for (index_t i = 0; i < loop_limit; i++) {
          if constexpr (is_beta_zero) {
            element_t data_right = *(scratch + i * sg_store_ld);
            *(C + i * ldc) = alpha_ * data_right;
          } else {
            element_t data_left = *(C + i * ldc);
            element_t data_right = *(scratch + i * sg_store_ld);
            *(C + i * ldc) = alpha_ * data_right + beta_ * data_left;
          }

        }
      }
    } else if (sg_mc < sg_rows) {
      if (sg_item_id < sg_mc) {
        // cl::sycl::ext::oneapi::experimental::printf("sg_id:%d | item_id:%d | mc:%d | nc:%d\n", sg_id, sg_item_id, sg_mc, sg_nc);
...
#pragma unroll
  ...
  template <bool check_m_limit, bool check_n_limit, typename InputPointerType,
            typename CType>
  SYCL_BLAS_INLINE void compute_block_gemm(const cl::sycl::nd_item<3> &id, 
                                           InputPointerType s2, InputPointerType s4, 
                                           CType& reg_res) noexcept {
    using namespace cl::sycl::ext::oneapi::experimental::matrix;
    using AType = joint_matrix<typename tile_type::jmInpType, matrix_use::a, sg_rows, tile_type::sg_K, matrix_layout::col_major>;
    using BType = joint_matrix<typename tile_type::jmInpType, matrix_use::b, tile_type::sg_K, sg_cols, matrix_layout::col_major>;
    ...
    for (index_t i = 0; i < cl_elems / tile_type::sg_K; i++) {
      joint_matrix_load(sg, inA, new_A, strideA); // M
      joint_matrix_load(sg, inB, new_B, strideB); // N

      reg_res = joint_matrix_mad(sg, inA, inB, reg_res);

      new_A += ldsa * tile_type::sg_K;
      new_B += tile_type::sg_K;
    }
  }
  ...
  static SYCL_BLAS_INLINE typename std::enable_if<db>::type sync_smem(
      const cl::sycl::nd_item<3> &id, index_t &ofs_sign, P &s,
      Ps &... ss) noexcept {
    s += ofs_sign * o;
  ...
  template <bool db>
  static SYCL_BLAS_INLINE typename std::enable_if<db>::type sync_smem(
      const cl::sycl::nd_item<3> &, index_t &ofs_sign) noexcept {
    ofs_sign = -ofs_sign;
  }

  template <bool db, index_t..., typename... Ps>
  static SYCL_BLAS_INLINE typename std::enable_if<!db>::type sync_smem(
      const cl::sycl::nd_item<3> &id, index_t &, Ps &...) noexcept {
    id.barrier(cl::sycl::access::fence_space::local_space);
  }
