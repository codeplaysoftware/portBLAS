...
#ifndef PORTBLAS_BLAS3_NO_LOCAL_PARTIAL_VEC_GEMM_HPP
#define PORTBLAS_BLAS3_NO_LOCAL_PARTIAL_VEC_GEMM_HPP
...
#ifdef BLAS_ENABLE_COMPLEX
  static_assert((VectorSize == 1 && is_complex_sycl<element_out_t>::value) ||
                    !is_complex_sycl<element_out_t>::value,
  ...
  PORTBLAS_INLINE void eval(cl::sycl::nd_item<1> id) noexcept {
    ...
    const auto boundary_check_m =
        [&](index_t dim_m_a_start)
            PORTBLAS_ALWAYS_INLINE { return dim_m_a_start < m; };
    const auto boundary_check_n =
        [&](index_t dim_n_b_start)
            PORTBLAS_ALWAYS_INLINE { return dim_n_b_start < n; };
...
#pragma unroll
  ...
  PORTBLAS_INLINE void compute_gemm_no_shared_pannel(
      ...
      const check_boundary_c_t &boundary_check_c, element_in_t *reg_a,
      element_in_t *reg_b, const bool out_of_range, const index_t &batch_stride,
...
#pragma unroll
  ...
  PORTBLAS_INLINE void compute_block_gemm_no_shared(
      element_in_t *reg_a, element_in_t *reg_b,
...
#pragma unroll
      for (int j = 0; j < item_rows / a_packet_size; j++) {
        if (do_check<check_block>(chk_boundary(dim_m_c_start + j * wg_rows,
          ...
          using l_vector_t = typename Packetize<a_packet_size, element_out_t,
                                                index_t>::PacketType;
          l_vector_t out_vec{0};
...
#endif  // PORTBLAS_BLAS3_NO_LOCAL_PARTIAL_VEC_GEMM_HPP
