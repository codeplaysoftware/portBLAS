...
void run_test(const combination_t<scalar_t> combi) {
  ...
  index_t size_x = size * incX;

  // Input/output vector
  std::vector<scalar_t> x_v(size_x);
  std::vector<ref_scalar_t> x_cpu_v(size_x);
  ...
  if constexpr (std::is_same_v<scalar_t, cl::sycl::half>) {
    ...
    std::transform(x_v.begin(), x_v.end(), x_cpu_v.begin(),
                   [](scalar_t x) { return (static_cast<ref_scalar_t>(x)); });
    // Reference implementation
    reference_blas::scal(size, alpha_f, x_cpu_v.data(), incX);
  } else {
    x_cpu_v = x_v;
    // Reference implementation
  ...
  auto copy_event =
      blas::helper::copy_to_device(q, x_v.data(), gpu_x_v, size_x);

  auto scal_event = _scal(sb_handle, size, alpha, gpu_x_v, incX, {copy_event});
  sb_handle.wait(scal_event);

  auto event = blas::helper::copy_to_host(q, gpu_x_v, x_v.data(), size_x);
