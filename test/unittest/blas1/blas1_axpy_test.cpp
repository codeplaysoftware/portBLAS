...
struct TestRunner {
  ...
  static void run_test(const combination_t<scalar_t> combi) {
    ...
    auto event =
        blas::helper::copy_to_host(q, gpu_y_v, y_v.data(), size * incY);
...
BLAS_REGISTER_TEST_CUSTOM_NAME(AxpyUSM, AxpyUSM, TestRunner<true>::run_test,
                               combination_t, combi, generate_name);
BLAS_REGISTER_TEST_CUSTOM_NAME(AxpyBuffer, AxpyBuffer,
