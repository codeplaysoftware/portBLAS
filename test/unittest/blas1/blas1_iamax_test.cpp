...
void run_test(const combination_t<scalar_t> combi) {
  ...
  blas::helper::copy_to_device(ex.get_queue(), x_v.data(), gpu_x_v, size * incX);
  auto gpu_out_s = blas::make_sycl_iterator_buffer<tuple_t>(int(1));
  blas::helper::copy_to_device(ex.get_queue(), &out_s, gpu_out_s, 1);

  _iamax(ex, size, gpu_x_v, incX, gpu_out_s);
  auto event = blas::helper::copy_to_host(ex.get_queue(), gpu_out_s, &out_s, 1);
  ex.wait(event);
