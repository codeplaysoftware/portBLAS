...
void run_test(const combination_t<scalar_t> combi) {
  ...
  auto event = blas::helper::copy_to_host(sb_handle.get_queue(), (a + offset), vR_gpu.data(),
                                                    size - offset);
...
inline BufferIterator<const scalar_t> func(
    BufferIterator<const scalar_t> buff,
    index_t offset) {
  return buff += offset;
}
