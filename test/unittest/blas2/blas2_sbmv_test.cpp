...
void run_test(const combination_t<scalar_t> combi) {
  ...
  index_t incY;
  index_t lda_mul;
  std::tie(alloc, n, k, alpha, beta, upper, incX, incY, lda_mul) = combi;
  ...
  // SYCL SBMV
  auto sbmv_event =
      _sbmv(sb_handle, *uplo_str, n, k, alpha, m_a_gpu, (k + 1) * lda_mul,
...
static std::string generate_name(
  ...
  T alpha, beta;
  bool upper;
  BLAS_GENERATE_NAME(info.param, alloc, n, k, alpha, beta, upper, incX, incY,
                     ldaMul);
}
