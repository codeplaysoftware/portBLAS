...
template <typename T>
using combination_t =
    std::tuple<std::string, int, int, int, int, T, T, bool, int, int, int>;
...
void run_test(const combination_t<scalar_t> combi) {
  ...
  index_t incY;
  index_t lda_mul;
  std::tie(alloc, m, n, kl, ku, alpha, beta, trans, incX, incY, lda_mul) =
      combi;
...
static std::string generate_name(
  ...
  T alpha, beta;
  bool trans;
  BLAS_GENERATE_NAME(info.param, alloc, m, n, kl, ku, alpha, beta, trans, incX,
                     incY, ldaMul);
}
