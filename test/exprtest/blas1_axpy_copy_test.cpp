...
TYPED_TEST(BLAS_Test, axpy_copy_test) {
  ...
  // creating the vector which the axpy result will copy into
  std::vector<scalar_t> vW(size, 0);

  // copy the axpy result vZ into vW
  for (int i = 0; i < size; ++i) {
    vW[i] = vZ[i];
  }
  ...
  _copy(ex, size, gpu_vY, 1, gpu_vW, 1);

  auto event = ex.get_policy_handler().copy_to_host(gpu_vW, vW.data(), size);
  ex.get_policy_handler().wait(event);

  // check that both results are the same
  for (int i = 0; i < size; ++i) {
    if (i % strd == 0) {
      ASSERT_NEAR(vW[i], vZ[i], prec);
    } else {
      ASSERT_EQ(vW[i], vZ[i]);
    }
  }
