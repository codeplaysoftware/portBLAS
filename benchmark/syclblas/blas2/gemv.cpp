...
std::string get_name(std::string t, int m, int n, std::string mem_type) {
...
void run(benchmark::State& state, blas::SB_Handle* sb_handle_ptr, int ti,
  ...
  std::vector<scalar_t> v_y_temp = v_y;
  {
    ...
    auto gemv_event = _gemv(sb_handle, *t_str, m, n, alpha, m_a_gpu, m, v_x_gpu,
...
void register_benchmark(blas::SB_Handle* sb_handle_ptr, bool* success,
                        std::string mem_type,
                        std::vector<blas2_param_t<scalar_t>> params) {
  for (auto p : params) {
    std::string ts;
    index_t m, n;
    scalar_t alpha, beta;
    ...
    auto BM_lambda = [&](benchmark::State& st, blas::SB_Handle* sb_handle_ptr,
      ...
      run<scalar_t, mem_alloc>(st, sb_handle_ptr, t, m, n, alpha, beta,
                               success);
    };
    benchmark::RegisterBenchmark(get_name<scalar_t>(ts, m, n, mem_type).c_str(),
...
void register_benchmark(blas_benchmark::Args& args,
  ...
  register_benchmark<scalar_t, blas::helper::AllocType::buffer>(
      sb_handle_ptr, success, "buffer", gemv_params);
#ifdef SB_ENABLE_USM
  register_benchmark<scalar_t, blas::helper::AllocType::usm>(
      sb_handle_ptr, success, "usm", gemv_params);
#endif
}
