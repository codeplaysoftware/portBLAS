...
void run(benchmark::State& state, blas::SB_Handle* sb_handle_ptr, int ti,
  ...
  MemBuffer<scalar_t> m_a_gpu(sb_handle_ptr, m_a.data(),
                              static_cast<size_t>(lda * n));
  MemBuffer<scalar_t> v_x_gpu(sb_handle_ptr, v_x.data(),
                              static_cast<size_t>(xlen));
  MemBuffer<scalar_t> v_y_gpu(sb_handle_ptr, v_y.data(),
  ...
  {
    MemBuffer<scalar_t> v_y_temp_gpu(sb_handle_ptr, v_y_temp.data(),
                                     static_cast<size_t>(ylen));
    cl_event event;
