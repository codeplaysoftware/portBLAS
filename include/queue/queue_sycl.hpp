...
class Queue_Interface<SYCL> {
  ...
  */
  template <typename T>
  inline cl::sycl::buffer<T, 1,  cl::sycl::detail::aligned_mem::aligned_allocator<unsigned char>> get_buffer(T *ptr) const {
    using pointer_t = typename std::remove_const<T>::type*;
    auto original_buffer =
        pointerMapperPtr_->get_buffer(static_cast<void *>(const_cast<pointer_t>(ptr)));
