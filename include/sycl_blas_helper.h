...
namespace blas {
  namespace helper {
    ...
    */
    ...
    inline cl::sycl::event copy_to_device(cl::sycl::queue q, 
        const element_t *src, BufferIterator<element_t> dst,
        size_t size) {
      auto event = q.submit([&](cl::sycl::handler &cgh) {
        auto acc =
          dst.template get_range_accessor<cl::sycl::access::mode::write>(cgh, size);
        cgh.copy(src, acc);
      });
      return event;
    }
    ...
    */
    template <typename element_t>
    inline
    cl::sycl::event copy_to_host( cl::sycl::queue q, 
        BufferIterator<element_t> src, element_t *dst,
        size_t size) {
      auto event = q.submit([&](cl::sycl::handler &cgh) {
        auto acc =
            src.template get_range_accessor<cl::sycl::access::mode::read>(cgh, size);
        cgh.copy(acc, dst);
      });
      return event;
    }
    ...
    template <typename element_t>
    inline cl::sycl::event fill(cl::sycl::queue q, 
        BufferIterator<element_t> buff, element_t value, size_t size) {
      auto event = q.submit([&](cl::sycl::handler &cgh) {
        auto acc = buff. template get_range_accessor<cl::sycl::access::mode::write>(cgh, size);
        cgh.fill(acc, value);
      });
      return event;
    }
