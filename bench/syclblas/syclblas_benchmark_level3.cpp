...
BENCHMARK(gemm, syclblas_level_3) {
  ...
  std::vector<scalar_t> a = benchmark<>::random_data<scalar_t>(m * k);
  std::vector<scalar_t> b = benchmark<>::random_data<scalar_t>(k * n);
  std::vector<scalar_t> c = benchmark<>::const_data<scalar_t>(m * n, 0);

  auto a_gpu = ex.get_policy_handler().template allocate<scalar_t>(m * k);
  auto b_gpu = ex.get_policy_handler().template allocate<scalar_t>(k * n);
  auto c_gpu = ex.get_policy_handler().template allocate<scalar_t>(m * n);
